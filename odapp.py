...
def google_login():
    ...
    try:
        ...
        st.error("Google OAuth configuration is missing. Check secrets.toml or Streamlit Cloud secrets.")
        st.stop()
...
def extract_data(template_text):
    ...
    carousell_id = ""  # Name will be mapped here
    item = ""
    color = ""
    size = ""
    status = "Pending"
    ...
    name_en = re.search(r'Name:\s*([^\n]+)', template_text)  # Map to Carousell ID
    ...
    carousell_id = name_en.group(1).strip() if name_en else (name_zh.group(1).strip() if name_zh else "")  # Map Name to Carousell ID
...
def order_details_page():
    ...
    if 'selected_order' not in st.session_state:
        st.error("No order selected. Go back to Pending Orders.")
        return
    ...
    st.write(f"**Carousell ID:**")
    st.text_area("", value=order_row['Carousell ID'], height=50, disabled=True, key="carousell_id_box")
    if st.button("Copy", key="copy_carousell_id"):
        copy_to_clipboard(order_row['Carousell ID'])
    st.write(f"**Phone:**")
    ...
    if st.session_state['page'] == 'Home':
        ...
        if st.session_state['show_button'] and st.button("Process and Add"):
            if template_text:
                ...
                if all([item, color, size, carousell_id, phone, address]):  # Check Carousell ID instead of Name
        ...
        query = st.text_input("Enter search terms (e.g., date, shoe model, Carousell ID)")
        if st.button("Search"):
            if query:
